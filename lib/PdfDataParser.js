...
import modulesPath from "./modulesPath.js";
...
export default class PdfDataParser extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    if (options.cells) {
      if (typeof options.cells === "number") {
        this.cellsRange.min = options.cells;
      }
      else if (typeof options.cells === "string") {
        let minmax = options.cells.split("-")
        if (minmax.length > 1)
          this.cellsRange.min = parseInt(minmax[ 0 ]);
        if (minmax.length > 2)
          this.cellsRange.max = parseInt(minmax[ 1 ]);
      }
    }

    if (options.RepeatHeading?.header || options[ "RepeatHeading.header" ] || options.header)
      this.cellsRange.heading = 1;
    ...
    this._cells = [];  // all cells parsed from document in x,y order
    this._headerRow = [];  // header row, i.e. column headers found on first page

    // output rows contain just value
    this._rows = [];       // array of output rows
  ...
  inCellRange(rowlen) {
    return (rowlen >= this.cellsRange.min && rowlen <= this.cellsRange.max) || (rowlen === this.cellsRange.heading);
  }
  ...
  filters(row) {
    ...
    if (output && this.rowNum === 1) {
      // first page
      if (this.page.pageNumber === this.firstPageNumber) {
        this._headerRow = row;
      }
      // subsequent pages
      else if (this.options.repeatingHeaders && this.rowsEqual(this._headerRow, row)) {
        output = false;
      }
    }
  ...
  */
  compareHeading(row, heading) {
    ...
    if (Object.prototype.toString.call(heading).slice(8, -1) === "RegExp")
      return heading.test(row[ 0 ].text);
    else
      return row[ 0 ].text === heading;
  ...
  async output(row) {
    ...
    for (let cell of row) {
      ...
      if (this.options.missingValues && row.length < this._headerRow.length && row.length >= this.cellsRange.min) {
        while ((col < this._headerRow.length && cell.x1 > this._headerRow[ col ].x2)
          || (col+1 < this._headerRow.length && cell.x1 > this._headerRow[ col + 1 ].x1)) {
          rowValues.push(null);
          col++;
        }
      }
    ...
    // check for missing cells at end
    if (this.options.missingValues && row.length < this._headerRow.length && row.length >= this.cellsRange.min) {
      while (rowValues.length < this._headerRow.length) {
        rowValues.push(null);
      }
    }

    if (this.listenerCount("data") > 0)
      this.emit("data", rowValues);
    else
